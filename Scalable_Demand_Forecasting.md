...
Let’s use those values in our workflow.
...
can also select best model per country. Default metric is ‘rmse’ but
this can be also specified through metric parameter inside
...
errors and evaluate best models from that perspective.
...
vis_table <- actual %>%
  ...
  rename(conf_low = .conf_lo.y,
         ...
         actual = .value.x,
         prediction = .value.y) %>%
  mutate(error = if_else(.key.y == 'prediction', 
                        actual - prediction, 
                        0),
         BIAS_cumsum = if_else(.key.y == 'prediciton',
                               cumsum(actual - prediction),
                               0)
  ) %>%
...
Let’s again look into best models, but this time let’s only inspect test
set horizon. It will improve readability of our plot.
...
vis_table %>%
  ...
  ggtitle('Best models fit on test set horizon.')
```
