...
```
Plots are busy as all models are displayed, however at at this stage we can also select best model per country. Default metric is 'rmse' but this can be also specified through metric parameter inside modeltime_nested_select_best() function.
...
With a little bit of data transformation we can get all we need to plot errors and evaluate best models from that perspective.
...
Let's again look into best models, but this time let's only inspect test set horizon. It will improve readability of our plot.
...
**Interesting observation from above plots is that best results (minimizing RMSE) are achieved through different models, depending on a country. This shows that 'one-fit all' approach doesn't work in demand forecasting.**
...
Indeed, distribution of errors in some countries is clearly skewed either to left or right. Extreme example of this is Ireland, which suggests presence of negative BIAS.<br/>
